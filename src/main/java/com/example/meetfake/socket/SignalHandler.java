...
import com.example.meetfake.service.SocketService;
...
public class SignalHandler extends TextWebSocketHandler {
	...
	public void afterConnectionClosed(final WebSocketSession session, final CloseStatus status)
		...
		if (rm != null) {
			...
			JSONObject info = (JSONObject) roomService.getInfo(rm).get(session);
			socketService.saveTracking(rm.getRoom_id(), info.get("id").toString(), info.get("start").toString(),
					getTime(), info.get("ip").toString());
			...
			for (Map.Entry<WebSocketSession, String> client : clients.entrySet()) {
				...
				sendMessage(client.getKey(), new WebSocketMessage(username, "leave", objectLeave.toString()));
				...
				sendMessage(client.getKey(), new WebSocketMessage(username, "user count", objectCount.toString()));
			}
		}
	...
	protected void handleTextMessage(final WebSocketSession session, final TextMessage textMessage) throws Exception {
		...
		Map<WebSocketSession, Object> info;
			...
			if (rm != null) {
				...
				for (Map.Entry<WebSocketSession, String> client : clients.entrySet()) {
					if (client.getKey().getId().equals(sid)) {
						...
						sendMessage(client.getKey(), new WebSocketMessage(username, "offer", objectOffer.toString()));
					}
				}
			}
			break;
			...
			if (rm != null) {
				...
				for (Map.Entry<WebSocketSession, String> client : clients.entrySet()) {
					if (client.getKey().getId().equals(sid)) {
						...
						sendMessage(client.getKey(), new WebSocketMessage(username, "answer", objectAns.toString()));
					}
				}
			}
			break;
...
//                System.out.println("ice " + session.getId());
			...
			if (rm != null) {
				...
				for (Map.Entry<WebSocketSession, String> client : clients.entrySet()) {
					if (client.getKey().getId().equals(sid)) {
						...
						sendMessage(client.getKey(), new WebSocketMessage(username, "ice", objectIce.toString()));
					}
				}
			}
			break;
		...
		case MSG_TYPE_JOIN:
			...
			info = roomService.getInfo(room);
			JSONObject objectInfo = new JSONObject();
			objectInfo.put("start", getTime());
			objectInfo.put("ip", dataBody.get("ip"));
			objectInfo.put("id", dataBody.get("id"));
			objectInfo.put("auth", dataBody.get("auth"));
			info.put(session, objectInfo);
			...
			sendMessage(session, new WebSocketMessage(username, "join", objectJoin.toString()));
			...
			for (Map.Entry<WebSocketSession, String> client : clients.entrySet()) {
				...
				sendMessage(client.getKey(), new WebSocketMessage("Bot", "text", objectMess.toString()));
			}
		...
		case MSG_TYPE_GET_CANVAS:
			if (rm != null) {
				if (roomService.getBoard(rm) != null) {
					...
					sendMessage(session, new WebSocketMessage(username, "getCanvas", objectBoard.toString()));
				}
			}
			break;
